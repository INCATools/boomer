        ...
        case (acc, ind) =>
          ...
          val maybeProbability = ZIO.fromOption(EntitySearcher.getAnnotations(proposalAnnotationSubject, ontology, HasProbabilityAP).asScala.to(Set).collectFirst {
            case Annotation(_, HasProbabilityAP, value ^^ XSDDouble) =>
              ZIO.fromOption(value.toDoubleOption).orElseFail(BoomErrorMessage(s"Proposal has probability value that can't be converted to a double: $proposalAnnotationSubject"))
          }).orElseFail(BoomErrorMessage(s"Can't find probability for proposal: $proposalAnnotationSubject")).flatten
